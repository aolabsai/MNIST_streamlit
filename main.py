...
def run_trials(is_training, num_trials, user_STEPS):
    ...
    if is_training:
        if st.session_state.app_type == "Black & White MNIST":
            ...
            INPUT1 = data.convolve(selected_in, data.gaussian_kernel)
    ...
    for t in np.arange(num_trials):
        ...
        if st.session_state.app_type == "Black & White MNIST":
            ...
            INPUT1 = data.convolve(selected_in[t, :, :], data.gaussian_kernel)
            INPUT1 = data.max_pooling(INPUT1)
...
def run_canvas():
    if st.session_state.app_type == "Black & White MNIST":
        ...
        input1 = data.convolve(st.session_state.canvas_image, data.gaussian_kernel)
        input1 = data.max_pooling(input1)
        input1 = data.bitmap_to_binary(input1).reshape(196*8)    
